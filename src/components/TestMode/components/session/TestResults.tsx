...
import { TestFeedback } from "../../../../api/model";
...
const TestResults: FC = () => {
  ...
  // Get test feedback from the session storage
  ...
  // While parsing the testFeedback, show a loading list
  if (!testFeedback) {
    return <LoadingList />;
  }
  return (
    <div className="m-auto flex w-full flex-col gap-y-6 duration-300 animate-in fade-in slide-in-from-left-40 sm:gap-y-8">
      ...
      {/**Header*/}
      <div className="flex flex-col justify-between gap-x-8 gap-y-6 text-gray-500 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-gray-800 ">
            Relatório
          </h1>
          <p className="text-lg">Teste {testId}</p>
        </div>

        <GradeBadge percentage={testFeedback.grade!} />
      </div>

      <div className="divide-y overflow-hidden rounded-lg">
        {testFeedback.gradeDistribution!.map((subtopicFeedback) => (
          <SubtopicResult
      ...
      <div className="flex w-full flex-col items-center justify-center gap-x-3 gap-y-2 pb-20  sm:flex-row">
        <Link to={`/test-mode/${testId}/review`} className="w-full sm:w-fit">
